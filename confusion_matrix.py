# From https://raw.githubusercontent.com/svpino/tf_object_detection_cm/master/confusion_matrix.py
...
def process_detections(detections_record, categories):
    ...
    for string_record in record_iterator:
        ...
        if decoded_dict:
            ...
            for c in groundtruth_classes: category_count[c] = category_count[c] + 1
...
def display(confusion_matrix, categories, category_count):
    print("\nConfusion Matrix:")
    ...
    for i in range(len(categories)):
        ...
        print('{}@{}IOU: precision:{:.2f} recall:{:.2f} ({} records)'.format(name, IOU_THRESHOLD, precision, recall, category_count[id+1]))
...
def main(argv):
    ...
    (confusion_matrix, category_count) = process_detections(FLAGS.detections_record, categories)

    display(confusion_matrix, categories, category_count)
