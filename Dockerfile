...
# Use DEBIAN_FRONTEND=noninteractive to avoid a tzdata configuration prompt.
RUN apt-get update -y && DEBIAN_FRONTEND=noninteractive apt-get install -y \
    protobuf-compiler python-pil python-lxml python-tk gpg-agent && \
    pip install Cython && \
...
# Configure tensorflow workspace for android
RUN echo "android_ndk_repository(\n\
    name=\"androidndk\",\n\
    path=\"/opt/android-ndk-r14b\",\n\
)\n\
...
# Local stuff for image downloading
RUN git clone https://github.com/GalAvineri/ISIC-Archive-Downloader.git && mkdir /isic && mv ISIC-Archive-Downloader /isic/isicdata
RUN pip install requests Pillow tqdm contextlib2 opencv-python-headless \
  scikit-image

ENV PYTHONPATH $PYTHONPATH:/isic:/isic/isicdata
